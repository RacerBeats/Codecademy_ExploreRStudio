...
kyoto_temps_f <- c(45,41,45,46,50,55,50)
...
temps_c <- c(F_to_C(kyoto_temps_f))
print(kyoto_temps_f)
print(temps_c)
...
is_nice_weather <- function(temp_list) {
  day_counter <- 0
  ...
  return(day_counter > length(temp_list)/2 )
}

#testing
is_nice_weather(kyoto_temps_f)
...
possible_destinations <- list(kyoto = kyoto_temps_f, bali = c(86,84,85,85,85,85,87), bahia = c(81,82,82,82,82,84,84))
...
#create data fram to store data for ggplot2
city_tempsdf <- data.frame(
  City = rep(names(possible_destinations), sapply(possible_destinations, length)), 
  Temperature = unlist(possible_destinations)
)

# create boxplot using ggplot 2
ggplot(city_tempsdf, aes(x = City, y = Temperature)) + geom_boxplot() +
  labs(
    title = "City temp boxplot",
    x = "City",
    y = "Temperature"
  ) +
  theme_bw()
